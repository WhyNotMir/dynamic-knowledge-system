...
_PDF_HEADER_FOOTER_ARTIFACT_RE = re.compile(
...
def _looks_like_pdf_major_heading(fragment: SourceFragment) -> bool:
    ...
    if not text:
        return False
    if _looks_like_pdf_header_footer_artifact(text):
        return False
...
def _looks_like_noise_fragment(fragment: SourceFragment) -> bool:
    ...
    if fragment.element_type == ElementType.CAPTION:
        return text.casefold() in {"<eos>", "eos"}
    ...
    if fragment.element_type == ElementType.PARAGRAPH and _looks_like_scientific_sentence(text):
        return False
...
def _looks_like_pdf_header_footer_artifact(text: str) -> bool:
    """Detect short source metadata lines without using domain vocabulary."""
    stripped = " ".join(text.split()).strip()
    if not stripped:
        return True
    if len(stripped) > 120:
        return False
    return bool(_PDF_HEADER_FOOTER_ARTIFACT_RE.search(stripped))


def _looks_like_scientific_sentence(text: str) -> bool:
    if len(text) < 80:
        return False
    ...
    return any(char in text for char in ".;,") or any(
        token in text for token in ("Table ", "Figure ", "Section ")
    )


def _candidate_body_fragments(fragments: list[SourceFragment]) -> list[SourceFragment]:
    body_types = {"paragraph", "list_item", "table", "quote", "code_block", "image", "footnote"}
    return [
        fragment
