...
DOCUMENT_HEADER_ARTIFACT_RE = re.compile(
...
def looks_like_real_heading(value: str) -> bool:
    ...
    if lowered in {"<eos>", "eos"}:
        return False
    if _looks_like_document_header_artifact(stripped):
        return False
...
def _looks_like_document_header_artifact(value: str) -> bool:
    stripped = normalize_text(value)
    if len(stripped) > 120:
        return False
    return bool(DOCUMENT_HEADER_ARTIFACT_RE.search(stripped))
